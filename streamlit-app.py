...
DATA_PATH = "churn_data_regulated.parquet"
...
def main():
    ...
    X_train = load_x_y("./data/X_train.csv")
    X_test = load_x_y("./data/X_test.csv")
    y_test = load_x_y("./data/y_test.csv")
    y_train = load_x_y("./data/y_train.csv")
    data_csv = load_x_y("Telco-Customer-Churn.csv")

    max_tenure = data_csv['tenure'].max()
    max_monthly_charges = data['Scaler__MonthlyCharges'].max()
    max_total_charges = data['Scaler__TotalCharges'].max()
    ...
    if election == "User-based SHAP":
        ...
        y_pred = model.predict(X_test.drop(columns = ['customerID']))
        st.write(f"Customer {customer_id}: CatBoost Model's prediction for the Customer Churn : {y_pred[customer_index]}")
        plot_shap(model, data, customer_id, X_train=X_train.drop(columns = ['customerID']), X_test=X_test.drop(columns = ['customerID']))
    
    # If Feature Importance is selected
    elif election == "Feature Importance":
        summary(model, data, X_train=X_train.drop(columns = ['customerID']), X_test=X_test.drop(columns = ['customerID']))
